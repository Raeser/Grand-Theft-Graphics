...
{
	ColorOutput output;
	UVPanner panner = UVPannerFunction(input.myUV.xy, DTHACK * 0.15f, 0.723f, 0.323f); //tested these floats in maya i dunno man 
...
{
	ColorOutput output;
	UVPanner panner = UVPannerFunction(input.myUV.xy, (DTHACK*0.65f) * 0.15f, 0.737f, -0.337f); //tested these floats in maya i dunno man 
	output.myColor.rgba = albedoTexture.Sample(textureSampler, float2(panner.pannedUV.x, 1 - panner.pannedUV.y)).rgba; //
	return output;
}
