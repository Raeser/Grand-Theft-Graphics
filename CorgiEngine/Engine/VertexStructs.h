...
{
public:
	void Init(int aMaxSize)
	{
		myData.Init(aMaxSize);
	}

	void PushVec4(Corgi::Vector4<float> aPos)
	{
		myData.Add(aPos.x);
		myData.Add(aPos.y);
		myData.Add(aPos.z);
	...
	void PushVec2(Corgi::Vector2<float> aPos)
	{
		myData.Add(aPos.x);
		myData.Add(aPos.y);
	}

	Corgi::GrowingArray<float> myData;
};